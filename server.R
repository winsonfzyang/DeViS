function(input, output, session) {
  ...
  observeEvent(input$datafile, {
    ...
    if (grepl("csv", file)) {
      ...
      values$maindata <- read_xlsx(file, na = c("NA",".", ""," "))
  ...
  output$xcol <- renderUI({
    ...
    items=names(df)
    names(items)=items
    selectizeInput("x", "x variable:",choices = items,
                   options = list(
  ...
  output$ycol <- renderUI({
    ...
    items=names(df)
    names(items)=items
    selectizeInput("y", "y variable(s):",choices=items,multiple=TRUE,
                   options = list(
  ...
  output$scatter_covar <-  renderUI({
    df <- values$maindata
    if (is.null(df)) return(NULL)
    items=names(df)
    selectInput('scatter_covar', 'Add covariates (optional)', choices = items, 
  ...
  outputOptions(output, "scatter_covar", suspendWhenHidden=FALSE)
  ...
  output$yaxiszoom <- renderUI({
    ...
    if ( !is.null(input$y)  ){
      if (is.null(df)| !is.numeric(df[,input$y] ) | (length(input$y) > 1 ) ) return(NULL)
      if (all(is.numeric(df[,input$y]) &&  (length(input$y) < 2 ) &&
              input$facetscalesin!="free_y"&&
              input$facetscalesin!="free")){
        yvalues <- df[,input$y][!is.na( df[,input$y])]
        ymin <- min(yvalues)
        ymax <- max(yvalues)
        ystep <- (ymax -ymin)/100
  ...
  output$mytablex = renderDataTable({
    df <- tabledata()  # need to change this to only dataset
    validate(       need(!is.null(df), "Please select a data set"))
  ...
  plotObject <- reactive({
    ...
    validate(need(input$x != '', "No variable(s) selected"))
    if(!is.null(plotdata)) {
      ...
      if (is.null(input$y) ) {
        ...
        if(is.numeric(plotdata[,input$x]) ){
          ...
          if ( input$mlineaddition){
            if (input$colorin == "None"){
              mu <- plotdata %>%
                summarise_(.dots = setNames(paste0('mean(',input$x, ')'), "grp.mean"))
              
              p <- p+
                geom_vline(data=mu, aes_string(xintercept=as.numeric(mu$grp.mean)),
                           linetype="dashed",size = 1.5)
              
            } else{
              mu <- plotdata %>%
                group_by_(input$colorin) %>%
                summarise_(.dots = setNames(paste0('mean(',input$x, ')'), "grp.mean"))
  ...
  output$ui_plot <-  renderUI({
    ...
    if(input$input_type == "ggplot"){
      plotOutput('plot',  width = "100%" ,height = input$height)
    } else{
      plotlyOutput('plotly.plot',  width = "100%" ,height = input$height)
    }
  })
  ...
  # Prepare covariates model
  # Covariates regression model
  covariate.fit.beta.obj <- reactive({
    
    df <- tabledata()
    req(input$y)
    
    if (!is.null(input$scatter_covar)) {
      covar_formula <- reformulate(c(input$scatter_covar),input$y)
      lm.covar <- lm(covar_formula, data=df)
    } else {
      print("no covariates in model")
    }
    
    
    
    # FUTURE UPDATE: THINK ABOUT STANDARDIZED COEFFS
    # IF (STANDARDIZE) []
    
  })
  
  # Covariate summary
  output$covariate.fit.beta <- renderPrint({
    
    # FUTURE UPDATE: THINK ABOUT STANDARDIZED COEFFS
    # IF (STANDARDIZE) []
    
    covar.obj <- covariate.fit.beta.obj()
    
    if (covar.obj=="no covariates in model") {
      print(covar.obj)
    } else {
      lm.beta.coavr.fit <- lm.beta(covar.obj)
      summary.lm.beta(lm.beta.coavr.fit)}
    
  })
  
  # X regression model
  fit.beta.obj <- reactive({
    df <- tabledata()
    
    # validate(need(!c(input$x,input$y),"no variables selected"))
    # if (input$x == "" | input$y == "" ) return(NULL)
    req(input$x, input$y)
    
    if (!is.null(input$scatter_covar)) {
      scatter_formula <- reformulate(c(input$scatter_covar,input$x),input$y)
    } else {
      scatter_formula <- reformulate(c(input$x),input$y)
    }
    ...
    lm.fit <- lm(scatter_formula, data=df)
    lm.fit
  })
  
  # X Regression summary
  output$fit.beta <- renderPrint({
    
    req(fit.beta.obj())
    lm.beta.fit <- lm.beta(fit.beta.obj())
    summary.lm.beta(lm.beta.fit)
  })
  
  # Compare models
  output$anova.model <- renderPrint({
    xreg.model <- fit.beta.obj()
    cov.model <- covariate.fit.beta.obj()
    
    if(cov.model=="no covariates in model"){
      print("There is model to compare")
    } else {anova(cov.model,xreg.model)}
  ...
  predicted_val <- reactive({
    req(fit.beta.obj())
    predict(fit.beta.obj())
  })
  
  
  # Adjusted value or raw value
  # choose_vals <- reactive({
  #   
  #   if (input$xvar == "" | input$yvar == "" ) return(NULL)
  #   if (input$adj_vals==TRUE) {y <- "predicted_val"}
  #   else
  #     y <- input$yvar
  #   y
  # })
  ...
  output$diag.reg <- renderPrint({
    model.df <- tabledata()
    model.reg <- fit.beta.obj()
    req(model.df,model.reg)
    diagnose_regression(model.reg,model.df)
  })
  
  # get the regression plots
  regression.plots <- reactive({
    model.reg <- fit.beta.obj()
    req(model.reg)
    RegressionPlots(model.reg)
  })
  
  # Studentized residual plots
  output$stdresd1 <- renderPlotly({
    req(regression.plots())
    regression.plots()[[1]]
    
  })
  
  output$stdresd2 <- renderPlotly({
    req(regression.plots())
    regression.plots()[[2]]
  ...
  output$DFFITSBETAs <- renderPrint({
    model.reg <- fit.beta.obj()
    req(model.reg)
  ...
  #     content = function(file) {write.csv(dstatsTable(), file)})
  ...
  sendmymail <- reactive({
    ...
    to <- "<winsonfz.yang@gmail.com>"
    
    subject0 <- isolate(input$subject)
    subject <- isolate(paste("New Message from:", name, email,
                             "regarding:", subject0, sep=" "))
    
    attachment0 <- isolate(input$attachment)
    attachment <- mime_part(attachment0$datapath,attachment0$name)
